...
class InstructorCardInfo {
   ...
   */
  constructor(courseSection, instrcutorList, timeLocationList) {
    this.courseSection = courseSection;
  ...
  static parseInstructorList = (courseInfo) => {
    return courseInfo.hasOwnProperty("instructor")
      ? courseInfo["instructor"].map(
          (instructor) =>
            instructor["commonName"] + " " + instructor["lastName"]
        )
      : [];
  };
    ...
    return new InstructorCampus(
      ...
      courseInfo.hasOwnProperty("instructor")
        ? courseInfo["instructor"]
            .reduce(
              (acc, curr) =>
                acc + " & " + curr["commonName"] + " " + curr["lastName"],
              ""
            )
            .slice(3)
        : "",
      // If campus exists, return the campus name as a string, eg: "Burnaby", else return empty string
      courseInfo.hasOwnProperty("courseSchedule") &&
      courseInfo["courseSchedule"][0].hasOwnProperty("campus")
  ...
  static removeDuplicatesFromList = (instructorCampusList) => {
    ...
    for (var i = 0; i < instructorCampusList.length; i++) {
      if (
        result.findIndex(
          (e) =>
            e.instructor === instructorCampusList[i].instructor &&
            e.campus === instructorCampusList[i].campus
        ) == -1
      ) {
...
class PreviousSemestersCardInfo {
   ...
   */
  constructor(termString, instructorCampusList, outlineLink) {
...
const getCourseInfoListFromSections = async (courseSectionJsons, urlString) => {
  let filteredCourseSections = courseSectionJsons.filter(
    (courseSection) => courseSection["classType"] == "e"
  ); // Filter out labs and tuiorials
  let courseSectionUrls = filteredCourseSections.map(
    (courseSection) => urlString + "/" + courseSection["value"]
  );
  return Promise.all(
    courseSectionUrls.map((url) =>
      fetch(url).then((response) => response.json())
    )
  );
};

const getPreviousSemestersInfos = async (
  currTermString,
  department,
  number
) => {
  ...
  while (iterateTerm != endTerm || iterateYear != endYear) {
    ...
    let courseInfos = await getCourseInfoListFromSections(
      courseSectionJsons,
      urlString
    );
    let instructorCampusList = courseInfos.map((courseInfo) =>
      InstructorCampus.getFromCourseInfo(courseInfo)
    );
    let previousSemesterCard =
      PreviousSemestersCardInfo.getFromCourseInfoAndICList(
        courseInfos[0],
...
const getCourseInfo = async (courseString) => {
  ...
  let courseSectionJsons = await fetch(urlString).then((response) => {
  ...
  let sectionSpecificInfos = courseInfos.map(
    (courseInfo) =>
      new SectionSpecificInfo(
        courseInfo["info"]["section"],
        courseInfo["info"]["courseDetails"],
        courseInfo["info"].hasOwnProperty("educationalGoals")
          ? courseInfo["info"]["educationalGoals"]
          : "",
        courseInfo.hasOwnProperty("requiredText")
          ? courseInfo["requiredText"]["details"]
          : "",
        courseInfo["info"].hasOwnProperty("materials")
          ? courseInfo["info"]["materials"]
          : "",
        courseInfo.hasOwnProperty("grades") ? courseInfo["grades"] : [],
        courseInfo["info"].hasOwnProperty("gradingNotes")
          ? courseInfo["info"]["gradingNotes"]
          : ""
      )
  );

  let instructorCards = courseInfos.map(
    (courseInfo) =>
      new InstructorCardInfo(
        courseInfo["info"]["section"],
  ...
  let previousSemestersInfo = await getPreviousSemestersInfos(
    term,
    department,
    number
  );
